...
namespace ClockScreenSaverGL.DisplayedObjects.Fonds.TroisD
{
    ...
    public class Vecteur2D
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]  public float Longueur()
        {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]	static public Vecteur2D operator *(float f, Vecteur2D v)     //produit par un réel
        { return new Vecteur2D(v.x * f, v.y * f); }

		[MethodImpl(MethodImplOptions.AggressiveInlining)]	public static Vecteur2D operator *(Vecteur2D v, float f)     //le prod par un float est commutatif !!!
        { return new Vecteur2D (v.x*f, v.y*f); }

		[MethodImpl(MethodImplOptions.AggressiveInlining)] public static Vecteur2D operator /(Vecteur2D v, float f) =>  (v * (1 / f));
        [MethodImpl(MethodImplOptions.AggressiveInlining)] static public float operator *(Vecteur2D v, Vecteur2D w)     //produit scalaire
        { return v.prodscal(w); }

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		static public Vecteur2D operator +(Vecteur2D v, Vecteur2D w) => new Vecteur2D(v.x + w.x, v.y + w.y);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]	static public Vecteur2D operator -(Vecteur2D v, Vecteur2D w)     //différence vectorielle
        { return new Vecteur2D(v.x - w.x, v.y - w.y); }

		[MethodImpl(MethodImplOptions.AggressiveInlining)]	static public Vecteur2D operator -(Vecteur2D v)     //negatif
         ...
         */
		[MethodImpl(MethodImplOptions.AggressiveInlining)]	public void Limiter(float max)
        {
            if (Longueur() > max)
            {
                Normalize();
