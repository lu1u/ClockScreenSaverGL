...
namespace ClockScreenSaverGL.DisplayedObjects.Fonds.TroisD
{
	internal class Moto : MateriauGlobal
	{
		...
		private Texture texturePaysage = new Texture();
		...
		{
			getConfiguration();
			texture.Create(gl, Configuration.getImagePath("texture_route.png"));
			texturePaysage.Create(gl, c.getParametre("Texture Paysage", Config.Configuration.getImagePath("paysage_route.png")));
		...
		public override void AfficheOpenGL(OpenGL gl, Temps maintenant, Rectangle tailleEcran, Color couleur)
		{
			...
			float[] col = { couleur.R / COULEUR, couleur.G / COULEUR, couleur.B / COULEUR, 1 };
			gl.Color(col);
			dessinePaysage(gl);
			brouillard(gl, col);
